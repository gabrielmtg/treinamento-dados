...
def generate_python_inferencer(num_inputs, h1, h2=8):
    ...
    def predict_sequential(X_list, weights):
        W1, b1, W2, b2, W3, b3 = weights
        outs = []
        for x in X_list:
            h1_arr = np.zeros(h1, dtype=np.float64)
            for i in range(h1):
                acc = np.float64(b1[i])
                for j in range(num_inputs):
                    acc += np.float64(W1[j, i]) * np.float64(x[j])
                h1_arr[i] = acc
            h1_arr = fann_sigmoid_symmetric(h1_arr)
            
            h2_arr = np.zeros(h2, dtype=np.float64)
            for i in range(h2):
                acc = np.float64(b2[i])
                for j in range(h1):
                    acc += np.float64(W2[j, i]) * h1_arr[j]
                h2_arr[i] = acc
            h2_arr = fann_sigmoid_symmetric(h2_arr)
            
            out_acc = np.float64(b3[0])
            for j in range(h2):
                out_acc += np.float64(W3[j, 0]) * h2_arr[j]
            outs.append(fann_sigmoid(np.array([out_acc]))[0])
        return np.array(outs)
    
    return parse_weights, predict_sequential
...
def main():
    ...
    clean_csv_path = os.path.join(base_path, "..", "..", "data")
    print(f"Loading native partitioned datasets from: {clean_csv_path}")
    def align_labels(samples):
        aligned = []
        for feats, label in samples:
            aligned.append((feats, 1.0 if label > 0.0 else 0.0))
        return aligned
        
    raw_attacks = align_labels(extract_raw_windows_from_file(os.path.join(clean_csv_path, "data_final_clean.csv")))
    raw_benign1 = align_labels(extract_raw_windows_from_file(os.path.join(clean_csv_path, "data_final_clean1.csv")))
    raw_benign2 = align_labels(extract_raw_windows_from_file(os.path.join(clean_csv_path, "data_final_clean2.csv")))
    
    all_raw = raw_attacks + raw_benign1 + raw_benign2
